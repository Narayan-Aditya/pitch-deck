...
// 11 — The ask
// ---------------------------------------------------------------------------
export function addAskSlide(pptx, { theme: t, brand, offerType, contact }) {
  const slide = newSlide(pptx, t, { dark: true });
  const offer = OFFER_VARIANTS[offerType] || OFFER_VARIANTS.podcast;
  ...
  // A named next step and a way to reach us is the whole job of a closing
  // slide — the reference deck ended on "Commercials — let's discuss." and gave
  // the reader nothing to act on.
  //
  // These lines were dropped for one commit, on the reasoning that no caller
  // passed a `contact`. That was true of the service this deck came from and
  // false here: the report page collects a booking link, an email and a phone
  // number, and its checklist tells people they are "on the last slide". A
  // field that is collected and then discarded is worse than one that was never
  // offered.
  const lines = [contact?.headline?.trim() || "Commercials — let's discuss."];
  const reach = [contact?.bookingLink, contact?.email, contact?.phone]
    .map((v) => String(v || '').trim())
    .filter(Boolean);
  ...
  text(slide, lines.join("\n"), {
    ...
    y: 4.05,
    w: 8.6,
    // Two lines need the room; one does not, and a fixed height would leave the
    // single-line case floating above the footer rule.
    h: reach.length ? 0.62 : 0.35,
    fontFace: CALIBRI,
    fontSize: 12,
    bold: true,
    lineSpacingMultiple: 1.25,
