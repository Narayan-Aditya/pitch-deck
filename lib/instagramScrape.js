...
async function igFetch(jar, url, headers) {
  ...
  try {
    ...
    if (/redirect count exceeded/i.test(cause)) {
      throw new Error('Instagram is throttling this login — wait several minutes before trying again');
    }
...
async function resolveUserId(jar, username) {
  ...
  if (res.status === 429) {
    throw new Error('Instagram is rate-limiting us — wait a few minutes and try again');
  }
...
async function fetchProfile(jar, userId) {
  ...
  if (res.status === 429) throw new Error('Instagram is rate-limiting us — wait a few minutes and try again');
