...
import { fetchViaApify, isConfigured as apifyConfigured } from './instagramApify.js';
...
/** Profile + recent posts, from browse2api if it can serve them and from Apify
 * if it can't.
 *
 * Two failures count as "can't", because nothing downstream sees a difference
 * between them: a thrown error (down, 401/402/403, upstream 502) and a 200 with
 * no profile on it — which is what a working-but-blocked scrape looks like from
 * here. A 404 is neither: that account genuinely does not exist, and spending an
 * actor run to be told so a second time helps nobody.
 *
 * With no Apify token configured, browse2api's own error is what surfaces.
 * Falling back should be invisible when it works and silent about itself when
 * it isn't possible; a message about a service the operator never set up would
 * only misdirect. */
async function fetchProfileAndPosts(username) {
  ...
  let data = null;
  try {
    data = await callBrowse2Api(username, since);
  } catch (err) {
    if (/no Instagram profile called/i.test(err.message || '')) throw err;
    primaryError = err;
  }

  if (data?.profile) return { ...data, source: 'browse2api' };

  if (!apifyConfigured()) {
    if (primaryError) throw primaryError;
    throw new Error('Instagram returned no profile data');
  }

  try {
    const fallback = await fetchViaApify(username, POSTS_LIMIT);
    if (fallback.profile) return fallback;
    // Apify agreeing there is nothing there is a real answer, not an outage.
    if (primaryError) throw primaryError;
    throw new Error('Instagram returned no profile data');
  } catch (err) {
    // Both backends are down. The primary one is the more actionable report —
    // Apify is the spare tyre, and "the spare is flat too" is not the headline.
    if (primaryError) throw primaryError;
    throw err;
  }
}
...
export async function scrapeInstagram(username) {
  const data = await fetchProfileAndPosts(username);
  ...
  return {
    ...
    // browse2api's profile object carries neither, and the report page's
    // grounding step degrades gracefully without them (see the check in
    // lib/openaiGenerate.js). Apify's does carry both, so when the fallback
    // answers they arrive for free.
    ...
    // Which backend answered. Worth surfacing: the two do not agree on every
    // field, and "the numbers changed" is easier to explain when the page can
    // say the source changed too.
    source: data.source || 'browse2api',
