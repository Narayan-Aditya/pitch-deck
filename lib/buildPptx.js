...
function addCreatorProofSlide(pptx, rd, brandName) {
  const matches = (rd?.creatorMatches || []).slice(0, 3);
...
// ==========================================
// PROOF — past work with real results. The highest-impact slide in the deck:
// every other claim reads as marketing until this one backs it up.
// ==========================================
function addProofSlide(pptx, settings) {
  const entries = (settings?.proof || []).filter(p => p.brand?.trim() && p.result?.trim()).slice(0, 3);
  ...
  const quote = settings?.testimonial?.quote?.trim();
  if (quote) {
    ...
    if (settings.testimonial.author?.trim()) {
      slide.addText(`— ${settings.testimonial.author}`, {
...
// ==========================================
function addInvestmentSlide(pptx, settings) {
...
// ==========================================
function addNextStepSlide(pptx, settings, brandName) {
  const ns = settings?.nextStep;
...
// same bytes.
// ==========================================
// Slides that depend on agency settings (proof, investment, next step) skip
...
export async function buildPitchDeckPptx(report, brandName, settings) {
  ...
  addProofSlide(pptx, settings);            // evidence
  addInvestmentSlide(pptx, settings);       // price + risk reversal
  addWhyPartnerSlide(pptx, brandName);
  addNextStepSlide(pptx, settings, brandName);   // the ask
