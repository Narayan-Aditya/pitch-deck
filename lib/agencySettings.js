...
export function loadSettings() {
  ...
  try {
    ...
    return {
      ...DEFAULT_SETTINGS,
      ...saved,
      testimonial: { ...DEFAULT_SETTINGS.testimonial, ...saved.testimonial },
      pricing: { ...DEFAULT_SETTINGS.pricing, ...saved.pricing },
      nextStep: { ...DEFAULT_SETTINGS.nextStep, ...saved.nextStep },
      proof: saved.proof?.length ? saved.proof : DEFAULT_SETTINGS.proof,
    };
  } catch {
...
export function settingsCompleteness(s) {
  const proofFilled = (s.proof || []).filter(p => p.brand?.trim() && p.result?.trim());
  return {
    hasProof: proofFilled.length > 0,
