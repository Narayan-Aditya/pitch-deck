...
export function toDeckArgs({ report, reportData, brand, offerType = "both" }) {
  return {
    ...
    // The report page calls this `nextStep`; the slide calls it the contact.
    // Passed straight through — the slide decides what to do with an empty one.
    contact: reportData?.nextStep || null,
  };
}
